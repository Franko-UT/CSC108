...
def section_needs_swap(currentstatus:str, ans:str, section_num:int) -> bool :
    currentstring = list (currentstatus)
    ansstring = list (ans)
    for i in range (HINT_MODE_SECTION_LENGTH ):
        temp = currentstring [ ( section_num ) * HINT_MODE_SECTION_LENGTH - 1 ]
        for j in range ( HINT_MODE_SECTION_LENGTH ):
            currentstring [ ( section_num ) * HINT_MODE_SECTION_LENGTH - 1 - j ] = currentstring [ ( section_num ) * HINT_MODE_SECTION_LENGTH - j - 2 ]
        currentstring [ ( section_num - 1 ) * HINT_MODE_SECTION_LENGTH ] = temp
        if currentstring [ ( section_num - 1 ) * HINT_MODE_SECTION_LENGTH : section_num * HINT_MODE_SECTION_LENGTH ] == ansstring [ ( section_num - 1 ) * HINT_MODE_SECTION_LENGTH : section_num * HINT_MODE_SECTION_LENGTH ]:
            return False
    return True

def get_move_hint (current_status:str, ans_game:str, section_num:int) -> str:
    if section_needs_swap (current_status,ans_game,section_num):
        return "SWAP"
    return "ROTATE"