...
of your code for the assignment should be in the wordlock_functions.py file.
"""
...
def start_game() -> tuple[str, str, str, str]:
    ...
    if in_test_mode(mode):
        ...
        section_len = wf.HINT_MODE_SECTION_LENGTH
    else:
    ...
    print(f"The section length is {section_len}")
...
def get_section_hint(state: str, answer: str) -> int:
    ...
    """
    section_nums = []
    for i in range(len(state) // wf.HINT_MODE_SECTION_LENGTH):
        section_nums.append(i + 1)
    random.shuffle(section_nums)
    for section_num in section_nums:
        if not wf.check_section(state, answer, section_num,
                                wf.HINT_MODE_SECTION_LENGTH):
            return section_num
...
def make_move(state: str, answer: str, section_num: int, move: str,
    ...
    >>> make_move('TCADOGEMUFOX', 'CATDOGEMUFOX', 1, 'R', 3)
    'ATCDOGEMUFOX'
    ...
    """
    if move == wf.CHECK:
        check_result = wf.check_section(state, answer, section_num, section_len)
        if check_result:
            print('The section is correct')
        else:
            print('The section is incorrect')
    else:
        state = wf.modify_section(state, move, section_num, section_len)
    return state
...
def get_section_number(answer: str, section_len: int) -> int:
    ...
    prompt = 'Enter a section number (1 - ' + \
             str(wf.get_num_sections(answer, section_len)) + '): '
    section_num = input(prompt)
    while not (section_num.isdigit()
               and wf.is_valid_section(int(section_num), answer, section_len)):
...
def get_move() -> str:
    ...
    """
    msg = 'Enter a move for that section (C to check, R to rotate, S to swap): '
        ...
        if hint_type == SECTION_HINT:
            ...
            if hint == 'Y':
                print('Your move hint is: '
                      + wf.get_move_hint(state, answer, section_num))
                return 1
    return 0
